...
const PharmacyManagement: React.FC = () => {
  ...
  const fetchPharmacies = async () => {
    ...
    try {
      ...
      const response = await fetch(`${baseUrl}/pharmacies?page=${currentPage}&limit=10`, {
        headers: {
  ...
  const handleSavePharmacy = async (pharmacyData: Partial<Pharmacy>) => {
    try {
      ...
      const url = editingPharmacy 
        ? `${baseUrl}/admin/pharmacies/${editingPharmacy.id}` 
        : `${baseUrl}/admin/pharmacies`;
  ...
  const handleDeletePharmacy = async (id: string) => {
    ...
    try {
      ...
      const baseUrl = 'https://health-agaba-be.onrender.com/api/v1';
      const response = await fetch(`${baseUrl}/admin/pharmacies/${id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
